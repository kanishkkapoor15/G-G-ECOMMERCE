...
app.use(cors());
app.use(express.static('upload/custom'));
...
//image handling for customized products
// Define storage for uploaded files
const customStorage = multer.diskStorage({
  destination: function (req, file, cb) {
      cb(null, 'upload/custom'); // Destination folder
  },
  filename: function (req, file, cb) {
...
//API for creating customize order request
...
const customOrder = mongoose.model('customOrder', customOrderSchema);
 ...
 // POST endpoint for processing custom orders
app.post('/process-custom-order', fetchUser, customUpload.single('productImage'), async (req, res) => {
  try {
      // Ensure req.user contains necessary properties
      if (!req.user || !req.user.email || !req.user.name) {
          return res.status(400).json({ error: 'User information not found in the token.' });
      ...
      let { userId, userEmail, userName, productQuantity, productType, productSize, productRemarks } = req.body;
      userId = req.user.id;
      userEmail = req.user.email;
      userName = req.user.name;
      ...
      const newOrder = new customOrder({
          ...
          userEmail,
          userName,
          productImage: req.file.path, // Use the path of the uploaded file
          productQuantity,
          productType,
          productSize,
          productRemarks
...
app.get('/all-orders', async (req, res) => {
  try {
    // Fetch all regular orders from the 'Order' collection
    const regularOrders = await Order.find({});
    // Fetch all custom orders from the 'customOrder' collection
    const customOrders = await customOrder.find({});
    // Return regular and custom orders as separate JSON responses
    res.status(200).json({ regularOrders, customOrders });
  } catch (error) {
    // Return an error message if there's any issue fetching orders
